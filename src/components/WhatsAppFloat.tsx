...
const WhatsAppFloat = memo(({ isMenuOpen = false }: WhatsAppFloatProps) => {
  ...
  return (
    <button 
      onClick={handleWA}
      className={`
        wa-float fixed bottom-6 right-6 z-40 flex items-center gap-3 
        bg-red-500 hover:bg-red-600 text-white px-5 py-3.5 rounded-full 
        font-bold text-sm shadow-xl hover:shadow-red-500/30 
        animate-float-pulse transition-all duration-300 cursor-pointer group
        ${isMenuOpen ? 'opacity-0 scale-90 pointer-events-none' : 'opacity-100 scale-100 pointer-events-auto'}
      `}
